...
int strongRecur(node *a[], int i){//현 노드와 연결된 최 상위 노드를 리턴함
    //변수선언
    node *t;
    int min, m, k, flag;
    //몇번째 방문한 노드인지.
    ...
    for (t = a[i]; t != NULL; t = t->next){//현 노드에 연결된 자식 노드가 없을때까지 실행
        if(marker[t->vertex]==0){//연결된 노드가 0인경우
            m = strongRecur(a, t->vertex);//연결된 노드의 값이
        }else{//연결된 노드에 이미 방문했으며 연결된 노드가 더 상위 노드인경우
            m = marker[t -> vertex];
        }
        if(m < min) {
                min = m;
...
void strongSearch(node *a[], int v){
    int i, n=0;
    node *t;
    //마커의 초기화
    for (i = 0; i < v; i++){
...
void inputAdjList(node * g[], int *v, int *e) {
	...
	for (j = 0; j < *e; j++) {
        ...
        //가지 분석해 리스트 만들기.
        i = nameToInt(vertex[0]);           //머리
		t = (node *)malloc(sizeof(node));
		t->vertex = nameToInt(vertex[1]);   //꼬리
		t->next = g[i];                     //머리에 꼬리연결
		g[i] = t;
	}
    printf("\n");
...
void depthFirstNonRecurList(node * a[], int v){
    ...
    for(i = 0; i < v; i++){ // 분리된 그래프들이 여러개가 있을수있으므로
        ...
        if (1 != marker[j]){//아직 방문 하지 않은경우에 푸시
                ...
                if(1 != marker[j]){ 
                    ...
                    for (p = a[j]; p != NULL; p = p -> next){               
                        if (0 == marker[p -> vertex]){  //아직 방문을 안하고 스택에 안들어간 노드만 스택에 넣는다    
                            push(p -> vertex);          
                            marker[p -> vertex] = 2;    //방문하지 않았고 스택에 들어간 노드는 2를 마크
                        }
                    }
                }
            }
            printf("\n >end searching one graph\n");
        }
    }
}
