...
void networkFlow(int cap[][MAXNODE], int flo[][MAXNODE], 
    ...
    while(getAugmentPath(resi,vrtx,s,t)){
        ...
        for (i = 1; path[i] > -1; i++){
            ...
            printMatrix(residual, vrtx);
    printMatrix(flow, vrtx);
        constructResidual(cap, flo, resi, vrtx);
    }
}
...
void inputAdjMatrix(int *v, int *e){
    ...
    for (j = 0; j < *e; j++){
        ...
        fscanf(fp, "%s %d %d", vertex, &tmpFlo, &tmpCap);

        //가지 분석해 리스트 만들기.
        capacity[nameToInt(vertex[0])][nameToInt(vertex[1])] = tmpCap;
...
int main(){
    int v, e;
    fp = fopen("networkFlowGraph.txt", "rt");
    inputAdjMatrix(&v, &e);
    printMatrix(capacity, v);
